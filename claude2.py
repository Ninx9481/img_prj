    ...
    def load_image(self):
        ...
        try:
            ...
            if use_auto:
                ...
                self.status_label.config(text="✓ Auto-crop ช่วงปอดสำเร็จ", fg="green")
            else:
                # manual crop ด้วยเมาส์
                if messagebox.askyesno(
                    "Crop ภาพ",
                    "ต้องการเลือกบริเวณลำตัว/ปอดด้วยตัวเองหรือไม่?",
                ):
                    if self.processor.manual_crop_interactive():
                        self.status_label.config(
                            text="✓ เลือกบริเวณสำเร็จ", fg="green"
                        )
                    else:
                        self.status_label.config(
                            text="✗ ไม่ได้เลือกบริเวณ", fg="red"
                        )

            # ถ้าไม่มี crop เลย ก็ใช้ทั้งภาพ
            self.step_images = [
