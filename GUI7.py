...
class XrayProcessorGUI:
    ...
    def _build_gui(self):
        ...
        ctk.CTkCheckBox(
            roi_row,
            text="Polygon ROI (click points on image; double-click to close)",
        ...
        # ค่า default (จะเปิด / ปิด CLAHE ตามนี้)
        self.params_default = {
            "use_hist_eq": True,
            "smooth_kernel": 5,
            "rib_length": 40,
    ...
    def _draw_roi_preview_on_canvas(self):
        ...
        # ถ้า polygon ปิดแล้ว ให้ต่อจุดสุดท้ายหากับจุดแรก
    ...
    def on_canvas_press(self, event):
        ...
        # ถ้าอยู่ในโหมด ROI: คลิกเพื่อเพิ่มจุด
    ...
    def process_image(self):
        ...
        try:
            ...
            # Step 0: original + ROI outline
            base0 = self._overlay_roi_if_any(self.processor.get_current_image().copy())
            self.step_images.append(("Original / ROI", base0))

            # hist eq / CLAHE
            if self.params["use_hist_eq"].get():
                self.processor.clahe_equalization()
                clahe_img = self._overlay_roi_if_any(self.processor.get_current_image().copy())
                self.step_images.append(("CLAHE (local histogram)", clahe_img))
            ...
            denoise_img = self._overlay_roi_if_any(self.processor.get_current_image().copy())
            self.step_images.append(
                (f"Denoise (median+bilateral+Gaussian, k={smooth_k})", denoise_img)
            )
            ...
            # bone mask images (ไม่ต้องวาด ROI เพิ่ม เพราะ ROI เป็นขอบของ mask อยู่แล้ว)
            ...
            self.step_images.append(("Final Result", final_img))
